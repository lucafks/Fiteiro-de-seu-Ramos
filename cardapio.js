...
const cardapio = {
    ...
    bolinhos: {
        titulo: "BOLINHOS",
        itens: [
            //{ nome: "BACALHAU", desc: "Bolinho crocante por fora e recheado com bacalhau.", preco: "20.00", img: "midia/produtos/bolinhodebacalhau.jpg" }
            // No seu array cardapio, adicione modelo3d:
{
                nome: "BACALHAU",
                desc: "Bolinho crocante por fora e recheado com bacalhau.",
                preco: "20.00",
                img: "midia/produtos/bolinhodebacalhau.jpg",
                modelo3d: "modelos/Copilot3D-45235a79-5270-4f1c-a342-052f2e893afe.glb"  // ← Adicione esta linha
            }, ,
    ...
    return `
        <div class="item-esculpido">
            ...
            <div class="item-info">
                ...
                <span class="preco-esculpido">${item.preco}</span>
            </div>
        </div>
    `;
}
...
function criarItem(item) {
    return `
        <div class="item-esculpido">
            <div class="foto-produto" style="position: relative; ${item.modelo3d ? 'cursor: pointer;' : ''}" 
                 ${item.modelo3d ? `onclick="event.stopPropagation(); abrirVisualizador3D('${item.modelo3d}', '${item.nome.replace(/'/g, "\\'")}')"` : ''} 
                 ${item.modelo3d ? `title="Clique para ver em 3D"` : ''}>
                <img src="${item.img}" alt="${item.nome}">
                ${item.modelo3d ? `
...
function gerarCardapio() {
    const main = document.querySelector('.moldura-tabua');

    // Remove todas as seções existentes
    const secoesExistentes = main.querySelectorAll('.categoria');
